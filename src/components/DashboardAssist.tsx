...
How can I help you today? You can try clicking one of the suggestion chips below!`,
 ...
 className="flex flex-col h-[calc(100vh-170px)] bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm font-sans relative max-w-5xl mx-auto w-full">

 {/* Chat header */}
 <div className="bg-white border-b border-slate-100 px-6 py-4 flex items-center justify-between relative z-10">
 ...
 <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/50 relative z-10">
 {messages.map((m) => {
 ...
 <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 animate-spin shrink-0">
 <Bot size={16} />
 </div>
 <div className="bg-white border border-slate-200 rounded-2xl rounded-tl-sm px-5 py-3.5 text-sm text-slate-500 animate-pulse shadow-sm">
 ...
 {/* Suggestion Chips */}
 <div className="px-6 py-3 bg-white border-t border-slate-100 flex flex-wrap gap-2 relative z-10 overflow-x-auto whitespace-nowrap">
 {suggestions.map((s) => (
 <button
 key={s}
 onClick={() => handleSendMessage(s)}
 disabled={loading}
 className="text-xs font-medium bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 px-3 py-2 rounded-full transition-colors disabled:opacity-50 cursor-pointer shadow-sm"
 >
 {s}
 </button>
 ))}
 ...
 disabled={loading}
 className="flex-1 bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-xl px-4 py-3 text-sm text-slate-900 focus:outline-none placeholder-slate-400 transition-all shadow-sm"
 />
 <button
 onClick={() => handleSendMessage()}
 disabled={loading || !input.trim()}
 className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-100 disabled:text-slate-400 disabled:border-slate-200 text-white px-6 rounded-xl font-medium transition-colors flex items-center gap-2 cursor-pointer shadow-sm"
 >
 Send <Send size={16} />
 </button>
 </div>
